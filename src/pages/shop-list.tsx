import { Box, Flex, Heading, Spinner, Divider, List, ListItem, Text, Checkbox, Stack } from '@chakra-ui/react';
...
export default function ShopList() {
    ...
    const changeItem = useMutation(async (data: { name: string, amount: number, bought: boolean, category: string }) => {
        const listItem = menuData.shopList.categories[data.category].find(grocery => grocery.name === data.name);
        listItem.bought = data.bought;
        return menuData.shopList;
    });

    function handleChange(event: ChangeEvent<HTMLInputElement>, listItem: { name: string, amount: number, bought: boolean }, category: string) {
                        ...
                        {error ? (
                            ...
                            <List as={Stack} spacing="2">
                                {Object.entries(menuData.shopList.categories)?.map(([key, groceries]) => (
                                    <Box key={key}>
                                        <Text fontSize="xl" color="gray.500" textTransform="capitalize">{key}</Text>
                                        {groceries.map((listItem, index) =>
                                            ...
                                            >
                                                <Flex>
                                                    ...
                                                    />
                                                    <Text pl="4" textDecoration={listItem.bought && 'line-through'}>{listItem.name} x{listItem.amount}</Text>
                                                </Flex>
                                                <AlertDialog
                                                    buttonProps={{
                                                        as: RiCloseLine,
