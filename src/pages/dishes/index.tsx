import { Box, Text, Flex, Heading, Button, Icon, Table, Thead, Tr, Th, Tooltip, Tbody, Td, useBreakpointValue, HStack, Spinner } from "@chakra-ui/react";
...
export default function DishList({ users, totalCount }) {
    ...
    const { data: useDishesData, isLoading, isFetching, error } = useDishes(page, {/*
        initialData: {
            users,
            totalCount
        },
    */});
    ...
    async function handlePrefetchDish() {
        await queryClient.prefetchQuery('dishes', async () => {
                                    ...
                                    <Tr key={dish.id}>
                                        <Td>
                                            <Box onMouseEnter={() => handlePrefetchDish()}>
                                                <Text fontWeight="bold" textTransform="capitalize">{dish.name}</Text>
                                            </Box>
                                        </Td>
